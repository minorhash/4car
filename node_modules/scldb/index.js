...
exports.insPal=function(nam,id,aid,mnt,ite,utc){
var stm=db.prepare('insert into pal values(?,?,?,?,?,?)');
stm.run(nam,id,aid,mnt,ite,utc)
};
...
exports.idPal=function(nam){
var stm=db.prepare('select *,rowid from pal where id=? order by rowid desc ');
var row=stm.get(nam);
return row;
};
...
exports.insrpy=function(nam,id,aid,mnt,ite,utc){
var stm=db.prepare('insert into rpy values(?,?,?,?,?,?)');
stm.run(nam,id,aid,mnt,ite,utc)
};

exports.iterpy=function(ite,tok){
var stm=db.prepare('update rpy set ite=? where tok=?');
stm.run(ite,tok);
};

// === rpy get ===
exports.getRpy=function(nam){
var stm=db.prepare('select *,rowid from rpy where nam=? order by rowid desc ');
var row=stm.all(nam);
return row;
};

exports.idrpy=function(nam){
var stm=db.prepare('select *,rowid from rpy where id=? order by rowid desc ');
var row=stm.get(nam);
return row;
};

exports.allrpy=function(nam){
var stm=db.prepare('select *,rowid from rpy where nam=? order by rowid desc ');
var row=stm.all(nam);
return row;
};
